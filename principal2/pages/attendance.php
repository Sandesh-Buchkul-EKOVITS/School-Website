...
$conn->query("DELETE FROM attendance WHERE class='$class' AND att_date='$date'");
    ...
    if(isset($_POST['att'])){
        foreach($_POST['att'] as $roll => $status){
            ...
            $conn->query("
                INSERT INTO attendance (roll_no,class,att_date,status)
...
if($class){
    ...
    $resOld = $conn->query("
        ...
        FROM attendance 
        WHERE class='$class' AND att_date='$date'
    ");
